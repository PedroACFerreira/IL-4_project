...
read_data <- function(file_path) {
  read_csv(file_path, show_col_types = FALSE) %>%
    mutate(across(c(animal, cell), as.factor),  # Convert animal and cell to factors
          ...
          across(c(GL, ML, PCL, total), log1p)
   #        across(c(cg1,il,mo,prl), log1p)
   #       across(c(total), log1p)
           )  
}
...
parameters <- c("GL","ML","PCL","total")
#parameters <- c("cg1","il","mo","prl")
#parameters <- c("total")
...
results$P.Corrected_Sidak <- ifelse(!is.na(results$P.Value), 1 - (1 - results$P.Value) ^ (nrow(results)-1), NA)
